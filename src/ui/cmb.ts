...
import { copyFileToDist, analyzeFiles, parseEntryAst, analyzeStyleFilesImport, CMB_OUTPUT_NAME, copyAllInterfaceFiles } from './common'

async function buildForCMB (uiIndex = 'index', buildData: IBuildData) {
  const {appPath} = buildData
  const compiler = new Compiler(appPath, uiIndex, true)
  console.log()
  console.log(chalk.green('开始编译 CMB 端组件库！'))
  await compiler.buildTemp()
  if (process.env.TARO_BUILD_TYPE === 'script') {
    await buildCMBScript(buildData)
  } else {
    await buildH5Lib(uiIndex, buildData)
  }
}

async function buildCMBScript (buildData: IBuildData) {
  ...
  outputDirName = `${outputDirName}/${CMB_OUTPUT_NAME}`
...
export { buildForCMB, buildH5Lib, buildCMBScript }
